...
export class FilterQueryBuilder extends React.Component<
  FilterQueryBuilderProps,
  FilterQueryBuilderState
> {
    ...
    return (
      <div>
        ...
        <Alert variant="info" hidden={!this.state.showInfo}>
          <p>
            <b>Multiple filter options</b> on the same step translate as an OR operation (rows that
            match one of them are returned), while adding multiple filter steps on an operation
            translates as an AND (rows that match both are returned).
          </p>

          <p>
            The example below explains how the <b>contains</b> operation works:
          </p>
          <p>
            Consider a <b>contains</b> operation for donor country
          </p>
          <ul>
            <li>
    ...
    if (filters) {
      return filters.map((filter, index) => (
        <FilterItem
          ...
          onDuplicateFilter={() => this.onDuplicateFilter(index)}
        />
      ));
    }
  ...
  private onDuplicateFilter = (index: number) => {
    if (this.props.filters) {
      let filter: FilterMap | undefined = this.props.filters.get(index);
      if (filter && this.props.onUpdateFilters) {
        filter = filter.set('value', '');
        const filters = this.props.filters.push(filter);
        this.props.onUpdateFilters(JSON.stringify(Map({ filters } as any).toJS()));
      }
    }
