...
export const fetchOperationData = ({ limit = 10, offset = 0, id }: FetchOptionsWithId): Partial<QueryDataAction> =>
  ({ type: FETCH_OPERATION_DATA, payload: { id, limit, offset } });
export const fetchOperationDataFailed = (alert: string): Partial<QueryDataAction> =>
  ({ type: FETCH_OPERATION_DATA_FAILED, alert });
export const setOperationData = (data: List<OperationDataAPIResponseMap>, payload: FetchOptions) =>
