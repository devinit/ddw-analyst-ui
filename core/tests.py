...
class TestPypikaUtils(TestCase):
    ...
    def test_can_generate_filter(self):
        expected = 'SELECT "crs_current".* FROM "public"."crs_current" WHERE "crs_current"."year">=1973 OR "crs_current"."short_description" ILIKE \'%sector%\' OR "crs_current"."short_description" ILIKE \'%wheat%\''
        OperationStep.objects.create(
            operation=self.op,
            step_id=2,
            name="Filter",
