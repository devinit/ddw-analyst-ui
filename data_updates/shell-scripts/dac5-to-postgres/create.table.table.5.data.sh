...
# Use this script to create the tables in the 'table_5_data' schema
...
db_name="oecd_dac_table_5_mirror"
release=$1
...
#*****************************************************************************#
  ...
  echo "CREATE SCHEMA IF NOT EXISTS "$db_name".table_5_data" | psql -d postgres -e


  echo -e "
    DROP TABLE IF EXISTS table_5_data.\"${release}\" CASCADE;
  ...
  echo -e "
    CREATE TABLE table_5_data.\"${release}\"
    (
    row_id             SERIAL
    , donor_code       SMALLINT NOT NULL CONSTRAINT valid_donor_code CHECK (donor_code BETWEEN 1 AND 21600)
    , donor_name       CHARACTER VARYING NOT NULL
    , sector_code      SMALLINT NOT NULL CONSTRAINT valid_sector_code CHECK (sector_code BETWEEN 100 AND 1000)
    , sector_name      CHARACTER VARYING NOT NULL
    , aid_type_code    INTEGER NOT NULL CONSTRAINT valid_aid_type_code CHECK (aid_type_code IN (528, 529, 530))
    , aid_type_name    CHARACTER VARYING NOT NULL
    , amount_type_code CHARACTER(1) NOT NULL CONSTRAINT valid_amount_type_code CHECK (amount_type_code IN ('A', 'D'))
    ...
    , value            NUMERIC
    , flags            TEXT
    , CONSTRAINT table_5_data_${release}_pkey PRIMARY KEY (row_id)
    );
  " | psql -A -t -e -d ${db_name};
