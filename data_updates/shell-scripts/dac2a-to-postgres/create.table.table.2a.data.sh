...
# Use this script to create the tables in the 'table_2a_data' schema
...
db_name="oecd_dac_table_2a_mirror"
release=$1
...
#*****************************************************************************#
  ...
  echo "CREATE SCHEMA IF NOT EXISTS "$db_name".table_2a_data" | psql -d postgres -e

  echo -e "
    DROP TABLE IF EXISTS table_2a_data.\"${release}\" CASCADE;
  ...
  echo -e "
    CREATE TABLE table_2a_data.\"${release}\"
    (
    row_id           SERIAL
    ...
    , aid_type_code  SMALLINT NOT NULL CONSTRAINT valid_aid_type_code CHECK (aid_type_code BETWEEN 106 AND 296)
    ...
    , value          NUMERIC
    , flags          TEXT
    , CONSTRAINT table_2a_data_${release}_pkey PRIMARY KEY (row_id)
    );
  " | psql -A -t -e -d ${db_name};
